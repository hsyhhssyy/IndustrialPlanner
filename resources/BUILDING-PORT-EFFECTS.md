...
网站新增管道、传送带材质和 Shader 留待后续讨论接入；本次没有替换现有物流绘制。当前发布清单未解析项为 0，7 处交付键按明确的单视图及模式声明关联，记录在 `normalizations` 中。

## 历史交付与实现记录（2026-09-11）

以下数量、失败记录及默认命令描述的是旧交付，不得直接用于重发当前网站素材。当前导入仅使用技能中的网站入口。

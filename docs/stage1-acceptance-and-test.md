...
- 退出仿真后停止推进并返回编辑模式。
- 退出仿真后清空全部运行态内容：系统外仓库库存、机器进度、机器内部物品、传送带在途物品等。
- 配方数据本身不发生变化。
...
操作：点击“传送带”模式，拖拽绘制路径创建连接；再切换到“管道”模式。

期望：

- 传送带模式可成功创建路径，路径在画布中可见并保留。
- 传送带可只铺设半段并保留，不要求必须形成 `out -> in`。
- 当路径形成 `out -> in` 时，系统会额外建立连接关系用于仿真。
- 从输出端口离开与进入输入端口时，方向必须满足端口 `direction`。
...
- 执行与“退出仿真”一致的全量运行态重置（系统外仓库库存、机器进度、机器内部物品、传送带在途物品等）。
- 统计数据同步重置。
...
- 端口在不同朝向下与建筑几何关系一致。
- 端口 `direction` 随旋转同步更新，且 90° 方向映射正确。
- 建筑旋转围绕几何中心执行，旋转后位置与尺寸变化符合中心不变原则。
...
- 桥接（交叉）节点两路可直行通过。
- 分流节点满足单入多出，汇流节点满足多入单出。
- 节点从 3 向扩展到 4 向且被本次绘制作为拐点/端点使用时，若不满足“单入或单出”则应拒绝。
- 传送带对撞（同轴双入/同轴双出）应被拒绝。
- 路径绘制过程中允许单步回退，但不允许沿已有同段继续铺设。
- 删除联通传送带可直接执行，无二次确认。
- 联通删除按 4 邻接定义执行（不含对角）。

## AC-09B 删除建筑不删传送带

操作：先铺设与建筑相连和不相连的多段传送带，再在删除模式删除单个建筑与“删除所有建筑”。

期望：

- 删除建筑后，画布中所有传送带格子与带段保持不变。
- 指向被删建筑的连接引用被移除，不出现悬挂连接。

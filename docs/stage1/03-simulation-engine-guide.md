...
- 可接入传送带或设备端口
- 不承担生产周期逻辑
- 当 `submitToWarehouse=true` 时，按仿真时间每 1 秒 触发一次批量提交，将该存储箱内所有物品一次性提交到仓库

## 6. 仿真控制行为

- **开始仿真**：初始化全部 runtime state、统计采样器，并重置仓库库存为统一初始值（`item_originium_ore=∞`，其他物品=`0`）
- **开始仿真（补充）**：对每个 `processor` 读取编辑态预置输入配置（`preloadInputItemId` + `preloadInputAmount`），按该设备输入缓存容量进行夹取后注入 `inputBuffer`

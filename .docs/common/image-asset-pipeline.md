...
1. 运行时资源文件名采用 `原名.哈希.webp` 形式，例如：`item_xxx.a1b2c3d4.webp`。
2. 哈希由源图片内容计算，源文件不变时哈希不变。
3. 只有以下场景会导致文件名哈希变化：
	- 源图片内容发生修改；
	- 新增了新的源图片。
4. 前端通过生成的映射表将逻辑路径（如 `/itemicon/item_xxx.webp`）映射到带哈希文件名的真实路径。
5. 不再依赖 URL 查询参数做缓存控制，避免 CDN 忽略查询参数导致缓存失效。
...
5. 资源缓存粒度是“单文件级别”，未改动资源不会因全局版本变化而失去缓存。

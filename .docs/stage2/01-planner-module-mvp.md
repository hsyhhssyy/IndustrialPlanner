...
> 冻结说明（2026-02-26）：本文件包含历史草案条目。Stage2 实际交付口径以 `README.md` 与 `01-scope-and-dod-stage2.md`（冻结版）为准；未实现条目已迁移至 `../stage3/`。
...
- 管道方向与容量规则：
  ...
  - 等效管道缓存池结算不改变方向约束，注入/抽取仍需满足端口方向匹配。
...
- 管道输送口径：
  ...
  - 等效管道缓存池不放宽方向约束，双端是否可注入/抽取取决于端口方向匹配。
...
24. 管道本体有方向：同一连通管道本体需按端口方向输送液体。
25. 管道分流器/汇流器有方向：方向不匹配时对应路径不运输。
26. 连通管道本体容量按 `2 × 管道长度` 计算，且注入/抽取需满足端口方向匹配。

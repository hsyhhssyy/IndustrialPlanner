...
- 管道传染（关键特性，正式生效）：
  - “管道系统”定义：由管道、储液罐及通过管道分流器/汇流器/桥接器连通的管段构成。
...
24. 管道传染规则生效：同一管道系统仅允许一种液体，异液输入在未排空前必须阻塞。
25. 桥接器按两条独立通道处理，传染不跨桥接器另一通道传播。
26. 管道系统与连通储液罐完全排空后，方可切换运输另一种液体。
27. 反应池支持 `3` 固体缓存位与 `2` 液体缓存位，且两类缓存互不干扰。
28. 反应池允许多配方并行生产；未选择配方时反应池不生产。
29. 反应池产物可按配方配置目标输出缓存/通道，并由对应端口送出。
30. 可切换形态设备在本系统中以独立新设备实现（如“种植机（水培）”），不实现运行时形态切换。
31. 水培设备需先补齐到领域模型，随后方可被放置、计算与仿真使用。
32. 物品类别已包含“液体”，且类型变更后的物品按液体规则参与运输。
33. 液体不可经传送带运输，固体不可经管道运输。
34. 传送带端口与管道端口互为非法接口，不发生跨介质连通。
35. 武陵扩展区域允许布置抽水泵、管道、储水罐与管路设备。
36. 抽水泵仅可放置在扩展区域，在基地内放置必须失败。
37. 抽水泵提供清水无限资源入口，需通过管道接入基地内网络后生效。
38. 紫晶矿已补充为矿物（固体）分类，不按液体规则处理。
39. 瓶装液体以隐藏物品建模，内部维护 20 个组合项（4 瓶型 × 5 液体）。
40. 通用物品列表不直接展示瓶装液体组合项，必须通过专用选择器选择。
41. 特定瓶型配方仅匹配对应组合项，不允许跨瓶型替代。
42. 瓶型无关配方在 UI 仅显示 1 条，但实现层按瓶型展开隐藏配方执行。
43. 涉及返瓶的瓶型无关配方，需按输入瓶型返还对应瓶子。
44. 武陵特定建筑存在摆放上限；天有烘炉达到 `N` 后不可继续放置。
45. 武陵上限仅作用于武陵地区，不影响四号谷地。
46. 达到上限后删除已放置建筑可恢复可用名额。
47. 武陵建筑上限 `N` 来自领域模型配置，未配置上限的建筑不受此规则影响。
48. 抽水泵输出效率由领域模型配置并参与流体吞吐结算。

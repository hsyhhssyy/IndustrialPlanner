...
| S2-SM-004B1 | 管道本体无方向 | 武陵地区且已形成连通管道本体 | 分别从不同端注入并在相反端抽取 | 任意端注入/抽取均可形成有效输送 | P0 |
...
| S2-EX-004A | 无方向管道双端并发 | 同一连通管道本体两端均有输入/输出端口 | 双端同时注入与抽取 | 系统按同一等效管道缓存池结算且无方向阻塞 | P1 |

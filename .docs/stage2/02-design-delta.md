...
- 管道方向规则：管道本体无方向，可从任意方向向任意方向输送液体。
- 管道容量规则：连通管道本体等效为一个“管道缓存池”（抽象模型，非设备“储液罐”），容量为 `2 × 管道长度`。
- 真实设备“储液罐”规则：定向，`1` 入口 + `1` 出口。
- 管道组件规则：新增管道分流器、管道汇流器、管道桥接器，语义对齐传送带版本。
- 分流/汇流方向规则：管道分流器与管道汇流器有方向，方向不匹配时对应路径不运输。
- 扩展区域规则（武陵）：
...
- 管道传染规则（正式）：
	...
	- 连通管道本体按等效管道缓存池结算，可双端注入/抽取；
	- 管道本体容量按 `2 × 管道长度` 计算（不含储液罐与设备内部缓存）；
	- 同一系统仅允许一种液体；
	- 系统内有液体残留时，异液输入必须阻塞；

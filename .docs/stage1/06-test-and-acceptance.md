...
| 2026-02-23 | 删除模式“整条传送带”点击普通设备可删除 | 切换到删除模式并选择“整条传送带”；分别点击传送带与普通设备（如粉碎机/存储箱） | 点击传送带删除其连通带；点击普通设备时应执行单设备删除，不应无效 | 已修复：整条删除仅对带类生效；命中非带类时回退为单设备删除 | PASS | 实现位置：`src/App.tsx`（单格删除分支增加带类判断，`wholeBelt` 下非带类改为单删） |

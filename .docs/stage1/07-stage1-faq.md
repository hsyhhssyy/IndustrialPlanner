...
- 默认值：`true`（即默认提交）。
- 含义：开启时按仿真时间每 10 秒批量提交一次该存储箱内所有物品到全局仓库统计池；关闭时不提交。
...
取货口新增配置项 `pickupIgnoreInventory`：

- 默认值为 `false`。
- 开启后，取货口可无视仓库库存持续取出所选物品，且成功出货不扣减仓库库存。
- 对应物品在统计面板“当前库存”列显示为 `∞`。

矿石特例：

- 若取货口选择了带 `矿石` tag 的物品，`pickupIgnoreInventory` 强制为 `true`。
- UI 表现为开关默认开启且禁用编辑（不可关闭）。
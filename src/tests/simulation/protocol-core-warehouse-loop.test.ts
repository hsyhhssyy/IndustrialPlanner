...
// Replacement: src/tests/fixtures/blueprints/simulation/protocol-core-warehouse-loop/index.json
// Risk: Low；断言与被测动作不变。
...
// Human Review: Required
// Original code:
// import { createWarehouseSlotLink, findSlot, getDevice, getTick } from "./blueprint-test-helpers";
import { findSlot, getDevice, getTick } from "./blueprint-test-helpers";
// AI-REMOVED 2026-09-14:
// Reason: 场景构造已批量固化为带版本的蓝图文件。
// Trigger: 用户要求测试通过蓝图文件装载场景，保留版本便于后续迁移。
// Evidence: 原构造表达式已解析为完整实体集合，按正式迁移规则保存。
...
// AI-REMOVED 2026-09-14:
// Reason: 固定仓库链接和配置已保存到完整蓝图，不再需要动态链接工厂的输入类型。
// Trigger: 夹具文件化后 TypeScript 检出未使用类型。
// Evidence: createProtocolCoreWarehouseSlotLinks 已归档，没有活动代码引用该类型。
// Replacement: src/tests/fixtures/blueprints/simulation/protocol-core-warehouse-loop/index.json。
// Risk: Low。
// Human Review: Required
// Original code:
...
describe("protocol core warehouse links", () => {
  it("ships independently from every configured output", async () => {
    ...
    // Human Review: Required
    // Original code:
    // createBlueprint("protocol-core-all-outputs", [
    //         createEntity(
    //           CORE_ID,
    //           "sp_hub_1",
    //           0,
  ...
  it("submits looped input cargo so the output keeps flowing", async () => {
    ...
    // Human Review: Required
    // Original code:
    // createBlueprint("protocol-core-e8-loop-to-s8", [
    //         createEntity(
    //           CORE_ID,
    //           "sp_hub_1",
    //           0,
  ...
  it("sinks ordinary warehouse loader input directly into the warehouse", async () => {
    ...
    // Human Review: Required
    // Original code:
    // createBlueprint("warehouse-loader-sink", [

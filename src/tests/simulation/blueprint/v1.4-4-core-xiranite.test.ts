...
const EXPECTED_XIRANITE_PER_MINUTE = 120;

describe("4核息壤产线 - 息壤粉稳态产量验证", () => {
  it("2 分钟预热后，1 分钟滑动窗口产出 >= 120 个息壤粉，持续 2 分钟", { timeout: 600_000 }, async () => {
    ...
    if (ovenIds.length === 0) {
      throw new Error("蓝图中未找到息壤烘炉 (xiranite_oven_1)");
    }

    console.log(
      `[v1.4-4-core-xiranite] 找到 ${ovenIds.length} 台息壤烘炉: [${ovenIds.join(", ")}]`,
    );
    ...
    console.log(
      `[v1.4-4-core-xiranite] ${results.length} 个滑动窗口全部通过，` +
      `总产出 ${totalProduced}，` +

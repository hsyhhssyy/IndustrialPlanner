...
// Reason: storage-fill-observe 已迁移为 normal 项目中的自组装蓝图测试，
//   不再从 public/blueprints/tests/storage-fill-observe.json 读取蓝图执行。
// Trigger: 用户要求“把他改成 自组装蓝图的测试，而不是蓝图文件测试，可以加到你上面那个两个测试的测试集里”。
// Evidence: 自组装回归已落在 src/tests/simulation/storage-multi-slot-routing.test.ts 的
//   createStorageFillObserveBlueprint() 用例中，并由 test:normal 执行。
// Replacement: src/tests/simulation/storage-multi-slot-routing.test.ts
// Risk: Low
...
describe.skip("存储箱满槽观察测试", () => {
  it("已迁移为 normal 项目的自组装蓝图测试", () => {
    // 迁移后的有效用例见 src/tests/simulation/storage-multi-slot-routing.test.ts。
  });
});

...
export const ITEM_FLUID_COLOR_SOURCE_URL = "https://hsyhhssyy.github.io/Endfield-Building-TopView-Assets/buildings/logistics/fluid-profiles.json";
...
describe("网站流体配色写入 Registry", () => {
  it("只替换既有 fluidColors，并保持液体四层、气体两层和幂等性", () => {
    const sourceUrl = "https://hsyhhssyy.github.io/Endfield-Building-TopView-Assets/buildings/logistics/fluid-profiles.json";
    const result = updateRegistryFluidColorsSource(registry, profiles, sourceUrl);
    expect(result).toMatchObject({ profileCount: 2, phaseCounts: { liquid: 1, gas: 1 } });
    expect(result.source).toContain(`ITEM_FLUID_COLOR_SOURCE_URL = "${sourceUrl}"`);
    ...
    expect(updateRegistryFluidColorsSource(result.source, profiles, sourceUrl).source).toBe(result.source);
  });
  ...
  it("拒绝来源层缺失、来源集合变化和 Registry 相态冲突", () => {
    ...
    const sourceUrl = "https://hsyhhssyy.github.io/Endfield-Building-TopView-Assets/buildings/logistics/fluid-profiles.json";
    expect(() => updateRegistryFluidColorsSource(registry.replace('id: "gas-a"', 'id: "gas-b"'), profiles, sourceUrl))
      .toThrow("Registry fluid item set differs");
    expect(() => updateRegistryFluidColorsSource(registry.replace('tags: ["gas", OTHER_TAG]', 'tags: ["liquid", OTHER_TAG]'), profiles, sourceUrl))
      .toThrow();
  });
});

...
// Reason: Legacy 多基地控制器已失去产品启动入口，控制器装配和所有可达调用已清理。
// Trigger: 用户要求清理无用的 Legacy 多基地实现，并加入单基地草稿箱。
// Evidence: SimulationActionImpl.start 固定 singleBase；区域会话已不由任何产品路径启动。
// Replacement: src/simulation/dense/host.ts 的区域合图；Legacy 保留单基地控制器。
// Risk: 原区域控制器不再可单独构造；底层会话测试仍在各自模块中保留。

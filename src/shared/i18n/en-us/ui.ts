const UI: Record<string, string> = {
    "action.applyValue": "Apply",
    "action.cancel": "Cancel",
    "action.cancelContinuousPlacement": "Cancel continuous placement",
    "action.cancelMove": "Cancel Move",
    "action.cancelPlacement": "Cancel",
    "action.clearPatch": "Clear Patch",
    "action.close": "Close",
    "action.collapse": "Collapse",
    "action.confirm": "Confirm",
    "action.confirmMove": "Confirm Move",
    "action.confirmPlacement": "Confirm Placement",
    "action.continuousPlacement": "Continuous placement",
    "action.copySelection": "Copy Selection",
    "action.deemphasizeBelt": "De-emphasize Belts",
    "action.deemphasizePipe": "De-emphasize Pipes",
    "action.deleteSelection": "Delete Selection",
    "action.enterFullscreen": "Enter Fullscreen",
    "action.exit": "Exit",
    "action.exitFullscreen": "Exit Fullscreen",
    "action.expand": "Expand",
    "action.open": "Open",
    "action.pause": "Pause",
    "action.redo": "Redo",
    "action.removeLink": "Remove Link",
    "action.removeLinks": "Remove Links",
    "action.resume": "Resume",
    "action.rotateMove": "Rotate",
    "action.rotatePlacement": "Rotate",
    "action.rotateSelection": "Rotate",
    "action.rotateView": "Rotate View",
    "action.saveBlueprint": "Save Blueprint",
    "action.showBelt": "Show Belts",
    "action.showPipe": "Show Pipes",
    "action.start": "Start",
    "action.step": "Step",
    "action.stop": "Stop",
    "action.switchTheme": "Switch Theme",
    "action.switchToNormalMarquee": "Switch to Normal Marquee",
    "action.switchToReverseMarquee": "Switch to Reverse Marquee",
    "action.toggleValue": "Toggle",
    "action.undo": "Undo",
    "action.zoomIn": "Zoom In",
    "action.zoomOut": "Zoom Out",
    "activeTool.blueprint-placement": "Blueprint Placement",
    "activeTool.dark-pipe-link": "Dark Pipe Link",
    "activeTool.logistics-placement": "Logistics Placement",
    "activeTool.marquee": "Marquee",
    "activeTool.move": "Move",
    "activeTool.select": "Select",
    "activeTool.single-placement": "Single Placement",
    "app.title": "Endfield Industrial System Simulator",
    "webDavSave.failed": "WebDAV save failed",
    "webDavSave.saving": "Saving to WebDAV",
    "webDavSave.syncFailed": "WebDAV sync failed",
    "webDavInitialSync.syncing": "Syncing data",
    "webDavInitialSync.canvasSyncing": "Syncing current canvas",
    "webDavInitialSync.canvasProgress": "Current canvas sync progress",
    "webDavInitialSync.failed": "Data sync failed",
    "webDavInitialSync.retry": "Retry",
    "webDavInitialSync.disable": "Disable WebDAV",
    "webDavStatus.open": "Edit WebDAV Settings",
    "webDavStatus.title": "WebDAV Sync Status",
    "webDavStatus.overview": "Sync Overview",
    "webDavStatus.syncEnabled": "WebDAV master switch",
    "webDavStatus.enabled": "Enabled",
    "webDavStatus.disabled": "Disabled",
    "webDavStatus.currentRunReason": "Current trigger",
    "webDavStatus.initialStage": "Initial sync stage",
    "webDavStatus.pendingLocalChanges": "Pending local changes",
    "webDavStatus.lastUpload": "Last upload",
    "webDavStatus.lastDownload": "Last download",
    "webDavStatus.network": "Network Requests",
    "webDavStatus.maxConcurrentRequests": "Maximum concurrency",
    "webDavStatus.activeRequests": "Active",
    "webDavStatus.queuedRequests": "Queued",
    "webDavStatus.tasks": "Sync Tasks",
    "webDavStatus.taskProgress": "Progress: {completed}/{total}",
    "webDavStatus.taskLastStarted": "Started: {time}",
    "webDavStatus.taskLastFinished": "Finished: {time}",
    "webDavStatus.lastError": "Latest error",
    "webDavStatus.runReason.startup": "Site startup",
    "webDavStatus.runReason.foreground": "Returned to foreground",
    "webDavStatus.runReason.interval": "Periodic check",
    "webDavStatus.runReason.local-change": "Local change",
    "webDavStatus.runReason.settings-change": "Settings change",
    "webDavStatus.runReason.manual": "Manual sync",
    "webDavStatus.stage.canvas": "Current canvas",
    "webDavStatus.stage.blueprints": "Blueprints",
    "webDavStatus.stage.modules": "Modules",
    "webDavStatus.stage.toolbox": "Toolbox",
    "webDavStatus.stage.ready": "Ready",
    "webDavStatus.task.canvas": "Current canvas",
    "webDavStatus.task.blueprints": "Blueprints and folders",
    "webDavStatus.task.modules": "Module balancing",
    "webDavStatus.task.toolbox": "Toolbox and production planning",
    "webDavStatus.task.background-documents": "Other base documents",
    "webDavStatus.task.directory-maintenance": "Remote directory maintenance",
    "webDavStatus.task.interval-check": "Small Check",
    "webDavStatus.task.device-registration": "Current device registration",
    "webDavStatus.task.remote-devices": "Remote device list",
    "webDavStatus.taskPhase.idle": "Not run",
    "webDavStatus.taskPhase.queued": "Queued",
    "webDavStatus.taskPhase.running": "Running",
    "webDavStatus.taskPhase.success": "Completed",
    "webDavStatus.taskPhase.error": "Failed",
    "webDavConflict.discovering": "Conflict detected. Determining its scope",
    "webDavConflict.applying": "Applying conflict resolutions",
    "webDavConflict.summary": "{count} sync items were found. Choose which version to keep for each.",
    "webDavConflict.kind.upload": "Upload (local changes)",
    "webDavConflict.kind.download": "Download (remote changes)",
    "webDavConflict.kind.conflict": "Conflict (both changed)",
    "webDavConflict.remoteUpdatedAt": "Remote upload time: {time}",
    "webDavConflict.unknownTime": "Unknown",
    "webDavConflict.apply": "Apply All Choices",
    "webDavConflict.batchUseLocal": "Use Mine for All",
    "webDavConflict.batchUseRemote": "Use Remote for All",
    "webDavConflict.stopSync": "Keep Both Local and Remote Data, and Turn Off Sync",
    "webDavConflict.itemLabel": "{type} - {name}",
    "webDavConflict.nameUnavailable": "Name unavailable",
    "webDavConflict.currentPlan": "Current Plan",
    "webDavConflict.type.base": "Base",
    "webDavConflict.type.blueprint": "Blueprint",
    "webDavConflict.type.blueprintFolder": "Blueprint Folder",
    "webDavConflict.type.module": "Module",
    "webDavConflict.type.moduleFolder": "Module Folder",
    "webDavConflict.type.moduleCanvas": "Module Canvas",
    "webDavConflict.type.moduleCanvasFolder": "Module Canvas Folder",
    "webDavConflict.type.productionPlanning": "Production Planning",
    "webDavConflict.type.syncData": "Synced Data",
    "debugLogDialog.clear": "Clear Logs",
    "debugLogDialog.copied": "Copied",
    "debugLogDialog.copy": "Copy Logs",
    "debugLogDialog.empty": "No debug logs have been captured yet.",
    "debugLogDialog.export": "Export Log File",
    "debugLogDialog.exporting": "Exporting…",
    "debugLogDialog.guidance": "If you are experiencing issues, click \"Export Log File\" above and send the downloaded file to technical support.",
    "debugLogDialog.maximize": "Maximize Debug Logs",
    "debugLogDialog.restore": "Restore Debug Logs",
    "debugLogDialog.title": "Debug Logs",
    "debugLogDialog.unavailable": "Log collection is unavailable. Make sure this browser supports SharedWorker.",
    "device.desktop": "Desktop",
    "device.mobile": "Phone",
    "device.tablet": "Tablet",
    "dialog.maximize": "Maximize",
    "dialog.restore": "Restore",
    "encyclopedia.back": "Back",
    "encyclopedia.category.all": "All",
    "encyclopedia.category.entities": "Devices",
    "encyclopedia.category.items": "Items",
    "encyclopedia.entityLabel": "Device",
    "encyclopedia.filter.excludeBottledLiquid": "Exclude Bottled Liquids",
    "encyclopedia.filter.label": "Filter",
    "encyclopedia.group.asInput": "As Input",
    "encyclopedia.group.asMachine": "Machines",
    "encyclopedia.group.asOutput": "As Output",
    "encyclopedia.group.liquidDismantle": "Liquid Unbottling",
    "encyclopedia.group.liquidFilling": "Liquid Bottling",
    "encyclopedia.home": "Encyclopedia",
    "encyclopedia.itemLabel": "Item",
    "encyclopedia.noRecipes": "No related recipes",
    "encyclopedia.noResults": "No results found",
    "encyclopedia.recentItems": "Recent Items",
    "encyclopedia.recipe.deviceConsumption": "Consumes",
    "encyclopedia.recipe.deviceConsumptionAny": "Consumes (Any)",
    "encyclopedia.recipe.gasEnvironmentSuffix": " Environment",
    "encyclopedia.searchPlaceholder": "Search items or devices…",
    "encyclopedia.viewDetails": "View Details",
    "encyclopediaPicker.title.entity": "Select Device",
    "encyclopediaPicker.title.entry": "Select Item or Device",
    "encyclopediaPicker.title.item": "Select Item",
    "pickerCustomFilter.naturalResources": "Natural Resources",
    "pickerCustomFilter.rawMaterialsAndProducts": "Raw Materials & Products",
    "feedbackDialog.bilibili.action": "Open Bilibili",
    "feedbackDialog.bilibili.description": "Best for quick comments, brief issue reports, or simple suggestions.",
    "feedbackDialog.bilibili.note": "It is easier, more visible, and usually more convenient for domestic users.",
    "feedbackDialog.bilibili.title": "Bilibili Feedback",
    "feedbackDialog.github.action": "Open GitHub",
    "feedbackDialog.github.description": "Best for complex issues, simulation mismatches, layout bugs, or anything that needs clear reproduction steps.",
    "feedbackDialog.github.note": "You can attach screenshots, logs, and longer explanations, which also makes follow-up tracking easier.",
    "feedbackDialog.github.title": "GitHub Feedback",
    "feedbackDialog.intro": "Choose where to send your report. GitHub is better for screenshots, logs, and longer descriptions; Bilibili is faster for quick reports and easier for users in mainland China.",
    "feedbackDialog.skland.action": "Open Skland",
    "feedbackDialog.skland.description": "Best for quick comments, brief issue reports, or simple suggestions.",
    "feedbackDialog.skland.note": "The official Arknights community, suitable for direct feedback and discussion.",
    "feedbackDialog.skland.title": "Skland Feedback",
    "feedbackDialog.qqgroup.action": "Join QQ Group",
    "feedbackDialog.qqgroup.description": "Join the Endfield Quantization/Simulation Feedback Group QQ:1071155059, and chat with developers and other players.",
    "feedbackDialog.qqgroup.note": "Click or scan to join",
    "feedbackDialog.qqgroup.qqNumber": "QQ: 1071155059",
    "feedbackDialog.qqgroup.qrcodeAlt": "Feedback QQ Group QR Code",
    "feedbackDialog.qqgroup.title": "Join QQ Feedback Group",
    "feedbackDialog.title": "Feedback",
    "helpDialog.description": "Help topics will be organized here as multiple tabs.",
    "helpDialog.empty": "There is no help content to show right now.",
    "helpDialog.maximize": "Maximize Help",
    "helpDialog.restore": "Restore Help",
    "helpDialog.tab.featureGuide": "Feature Guide",
    "helpDialog.tab.gettingStarted": "Getting Started",
    "helpDialog.tab.shortcuts": "Controls",
    "helpDialog.tab.versionUpdates": "Version Updates",
    "helpDialog.title": "Help",
    "inspector.slotConfig.capacity": "Capacity",
    "inspector.slotConfig.clearSlot": "Clear Slot",
    "inspector.slotConfig.group": "Slot Group",
    "inspector.slotConfig.locked": "Locked",
    "inspector.slotConfig.selectItem": "Select Item",
    "inspector.submitToWarehouse.countdown": "Next Submit Countdown",
    "inspector.submitToWarehouse.label": "Wireless Submit to Warehouse",
    "inspector.warehouseItemLink.description": "Choose a warehouse item for each slot.",
    "inspector.warehouseItemLink.ignoreStock": "Ignore Stock",
    "inspector.warehouseItemLink.selectItem": "Select Warehouse Item",
    "label.currentTickInternalData": "Current Tick Internal Data",
    "label.currentTickSnapshot": "Current Tick Read Model",
    "label.definition": "Definition",
    "label.definitions": "Definitions",
    "label.documentValue": "Document Baseline",
    "label.effectiveValue": "Effective Value",
    "label.entityId": "Entity ID",
    "label.items": "Items",
    "label.links": "Links",
    "label.mode": "Mode",
    "label.multiSelectionSummary": "Multiple selection currently shows shared actions only; narrow it to one entity for detailed fields.",
    "label.noConfigFields": "No configurable fields in the scaffold yet.",
    "label.noConnections": "No explicit links on the current selection.",
    "label.noDiagnostics": "No diagnostics at the moment.",
    "label.noSelection": "No Selection",
    "label.position": "Position",
    "label.recipes": "Recipes",
    "label.rotation": "Rotation",
    "label.runtime": "Runtime",
    "label.runtimeDetailPlaceholder": "The runtime query lane will populate richer on-demand details here.",
    "label.runtimePatch": "Runtime Patch",
    "label.runtimePatchClearsOnExit": "Leaving simulation clears runtime patches and keeps the document unchanged.",
    "label.runtimePatchDisabled": "This field cannot be overridden during simulation.",
    "label.runtimePatchNone": "None",
    "label.touchPlacementHint": "Drag the ghost, then tap confirm to place it.",
    "leftDock.activeTool": "Tool",
    "leftDock.collapsed": "Library",
    "leftDock.currentMode": "Panel",
    "leftDock.title": "Left Context Panel",
    "locale.en-US": "English (AI Translate)",
    "locale.zh-CN": "Chinese",
    "mode.edit": "Edit",
    "mutability.document-only": "Document Only",
    "mutability.recompile-required": "Recompile Required",
    "mutability.runtime-mutable": "Runtime Mutable",
    "rightDock.base": "Base",
    "rightDock.collapsed": "Inspector",
    "rightDock.power": "Power",
    "rightDock.selection": "Device Properties",
    "rightDock.simulation": "Simulation",
    "rightDock.title": "Right Inspector Panel",
    "screen.landscape": "Landscape",
    "screen.portrait": "Portrait",
    "screen.square": "Near Square",
    "section.configFields": "Config Fields",
    "section.connections": "Connections",
    "section.diagnostics": "Diagnostics",
    "section.quickActions": "Quick Actions",
    "section.runtimeDetails": "Runtime Details",
    "section.runtimePatch": "Runtime Patch",
    "settingsAction.resetAllSettings": "Reset Settings",
    "settingsAction.resetAllSettingsConfirm": "Are you sure you want to reset all settings to defaults?",
    "settingsAction.resetOperationAndShortcuts": "Reset Operation Mode & Shortcuts",
    "settingsAction.resetOperationAndShortcutsConfirm": "Are you sure you want to reset operation mode and shortcuts to defaults?",
    "settingsAction.resetOperation": "Reset Operation Mode",
    "settingsAction.resetOperationConfirm": "Are you sure you want to reset operation mode to defaults?",
    "settingsDialog.groups": "Setting Groups",
    "settingsDialog.title": "Settings",
    "settingsField.debug-backend-api-address-override": "Backend Address Override",
    "settingsField.debug-backend-api-address-overrideDescription": "Override the backend API address when filled. Empty or whitespace-only values use the default endfield-api.amiyabot.com.",
    "settingsField.debug-show-fps": "Show FPS/TPS",
    "settingsField.debug-show-fpsDescription": "Show a collapsible FPS/TPS indicator panel at the top-left corner of the canvas when enabled.",
    "settingsField.debug-show-gesture-diagnostics-window": "Show Gesture Test Window",
    "settingsField.debug-show-gesture-diagnostics-windowDescription": "Show a debug tool for testing mouse and keyboard operations",
    "settingsField.debug-simulation-worker-detailed-report": "Detailed Simulation Worker Reporting",
    "settingsField.debug-simulation-worker-detailed-reportDescription": "Report detailed simulation diagnostics in the logs. This severely degrades performance and is intended only for diagnosing simulation errors whose cause cannot otherwise be identified.",
    "settingsField.game-always-show-grid-lines": "Always Show Grid Lines",
    "settingsField.game-always-show-grid-linesDescription": "When enabled, grid lines are always visible. When disabled, they only appear in specific modes near the current preview.",
    "settingsField.game-collapse-device-modes": "Collapse Device Modes",
    "settingsField.game-collapse-device-modesDescription": "Collapse variants of the same device into one button and select modes from its right cap.",
    "settingsField.game-show-pipe-exact-fluid-position": "Show Exact Pipe Fluid Position",
    "settingsField.game-show-pipe-exact-fluid-positionDescription": "Precisely show the current position of items inside pipes.",
    "settingsField.game-arknights-allow-empty-logistics-endpoints": "Allow Empty Logistics Starts",
    "settingsField.game-arknights-allow-empty-logistics-endpointsDescription": "When enabled, conveyors and pipes can start from empty cells. Otherwise they must start from a device with an output.",
    "settingsField.game-arknights-auto-create-splitters-and-convergers": "Auto Create Splitters/Convergers",
    "settingsField.game-arknights-auto-create-splitters-and-convergersDescription": "Automatically create splitters and convergers when conveyors or pipes are drawn into junctions.",
    "settingsField.game-arknights-copy-while-moving": "Copy While Moving",
    "settingsField.game-arknights-copy-while-movingDescription": "When enabled, Ctrl-click while moving places a copy at the current position, and touch controls show a Copy button.",
    "settingsField.game-arknights-immediate-marquee": "Immediate Marquee",
    "settingsField.game-arknights-immediate-marqueeDescription": "Mouse mode only: immediately start marquee selection when dragging from empty canvas without a long press.",
    "settingsField.game-arknights-immediate-move": "Immediate Move",
    "settingsField.game-arknights-immediate-moveDescription": "When dragging from a selected device, immediately trigger move without a long press.",
    "settingsField.game-arknights-inspector-open-on-second-click": "Open Inspector On Second Click",
    "settingsField.game-arknights-inspector-open-on-second-clickDescription": "When enabled, the first click only selects the device. Click the selected device again to open the properties panel.",
    "settingsField.game-arknights-operation-mode": "Arknights Operation Mode",
    "settingsField.game-arknights-operation-modeDescription": "Use the same operation mode and shortcuts as the game; this setting is currently unavailable.",
    "settingsField.game-arknights-selection-right-dock-sync": "Sync Right Dock With Selection",
    "settingsField.game-arknights-selection-right-dock-syncDescription": "Effective when \"Use Right Panel For Device Properties\" is enabled. Automatically expand the right panel on selection when it is not already open.",
    "settingsField.game-quick-place": "Quick Place",
    "settingsField.game-quick-placeDescription": "When enabled, use a shortcut to open the quick place popup at the mouse position.",
    "settingsField.game-show-device-icons": "Show Device Icons",
    "settingsField.game-show-device-iconsDescription": "Show icons on devices; locked on when simplified device icons are enabled.",
    "settingsField.game-show-device-names": "Show Device Names",
    "settingsField.game-show-device-namesDescription": "Show name labels on devices.",
    "settingsField.game-show-grass-background": "Grass Background",
    "settingsField.game-show-grass-backgroundDescription": "Use a grass texture on the canvas background.",
    "settingsField.game-show-hotkeys": "Show Hotkeys",
    "settingsField.game-show-hotkeysDescription": "Display shortcut key hints on interface buttons.",
    "settingsField.game-use-blueprint-style-device-images": "Use Blueprint Style Device Images",
    "settingsField.game-use-blueprint-style-device-imagesDescription": "Display device images in blueprint style; this does not improve performance.",
    "settingsField.game-use-inspector-panel": "Use Right Panel For Device Properties",
    "settingsField.game-use-inspector-panelDescription": "When enabled, use the right panel for device settings instead of a dialog.",
    "settingsField.other-debug-mode": "Debug Mode",
    "settingsField.other-debug-modeDescription": "Enable debug mode",
    "settingsField.other-experimental-features": "Enable Experimental Features",
    "settingsField.other-experimental-featuresDescription": "Enable unstable experimental features.",
    "settingsField.other-experimental-features-warning-confirm": "Enable Experimental Features",
    "settingsField.other-experimental-features-warning-message": "Experimental features are extremely unstable. Enabling them may corrupt your data or cause data loss. In the worst case, you may need to reset the website to continue using its features. Are you sure you want to enable experimental features?",
    "settingsField.other-experimental-features-warning-title": "Warning",
    "webDavConfig.enabled": "Enable WebDAV Sync",
    "webDavConfig.enabledDescription": "Sync base documents, blueprints, module balancing data, and production planning data across your devices through WebDAV.",
    "webDavConfig.title": "WebDAV Settings",
    "webDavConfig.url": "WebDAV URL",
    "webDavConfig.urlDescription": "Enter the parent WebDAV directory. The app will create an industrial-planner directory inside it.",
    "webDavConfig.username": "Username",
    "webDavConfig.usernameDescription": "The username used to connect to the WebDAV service.",
    "webDavConfig.password": "Password",
    "webDavConfig.passwordDescription": "The password or app-specific password used to connect to the WebDAV service.",
    "webDavConfig.maxConcurrent": "Maximum Concurrency",
    "webDavConfig.maxConcurrentDescription": "Limits concurrent WebDAV network requests. Higher values can sync faster but increase server load.",
    "webDavConfig.deleteAllData": "Delete All Data",
    "webDavConfig.deleteAllDataConfirm": "Are you sure you want to delete all WebDAV sync data on the server? This cannot be undone.",
    "webDavConfig.deleteAllDataFinalTitle": "Final Confirmation",
    "webDavConfig.deleteAllDataFinalPrompt": "Type \"Delete all saved content on the server\" to confirm",
    "webDavConfig.deleteAllDataFinalConfirm": "Confirm Delete",
    "webDavConfig.deleteAllDataDeleting": "Deleting all data on the server, please wait…",
    "cloudflareStatus.title": "Cloudflare Sync Status",
    "cloudflareStatus.open": "Edit Cloudflare Settings",
    "cloudflareStatus.syncEnabled": "Cloudflare Sync",
    "cloudflareStatus.phase": "Cloudflare Sync Status",
    "cloudflareStatus.smallCountdown": "Next Small Check",
    "cloudflareStatus.countdownNext": "in {time}",
    "cloudflareStatus.spaceSettings": "Cloudflare Space Settings",
    "cloudflareStatus.spaceName": "Space Name",
    "cloudflareStatus.spaceNameDescription": "Use the same space name on different devices to share data. Changes take effect only after saving.",
    "cloudflareStatus.saveSpaceName": "Save",
    "cloudflareStatus.savingSpaceName": "Saving…",
    "cloudflareStatus.spaceNameSaveFailed": "Failed to save the space name. Try again.",
    "cloudflareStatus.deleteAllData": "Delete All Remote Data",
    "cloudflareStatus.deleteAllDataConfirm": "This will clear all sync data in the Cloudflare remote space. This cannot be undone. Continue?",
    "cloudflareStatus.deleteAllDataFinalPrompt": "Type \"DELETE\" to confirm",
    "cloudflareStatus.abortCurrentTransaction": "Abort Current Transaction",
    "cloudflareStatus.abortingTransaction": "Aborting current transaction…",
    "webDavConfig.testConnection": "Test Connection",
    "webDavConfig.applySettings": "Apply Settings",
    "webDavConfig.testing": "Testing connection…",
    "webDavConfig.testSuccess": "Connection successful",
    "webDavConfig.testFailed": "Connection failed",
    "settingsField.experimental-webdav-status": "WebDAV Sync Status",
    "settingsField.experimental-webdav-statusDescription": "Shows the latest sync timestamps, connection state, and detailed task activity.",
    "settingsField.experimental-webdav-status-idle": "Idle",
    "settingsField.experimental-webdav-status-uploading": "Uploading",
    "settingsField.experimental-webdav-status-downloading": "Downloading",
    "settingsField.experimental-webdav-status-error": "Error",
    "settingsField.experimental-webdav-last-upload": "Last upload: {time}",
    "settingsField.experimental-webdav-last-download": "Last download: {time}",
    "settingsField.experimental-webdav-devices": "Remote devices",
    "settingsField.experimental-webdav-conflict-title": "Sync Conflict",
    "settingsField.experimental-webdav-conflict-message": "The remote version conflicts with your local version.",
    "settingsField.experimental-webdav-conflict-use-local": "Use Mine",
    "settingsField.experimental-webdav-conflict-use-remote": "Use Remote",
    "settingsField.experimental-webdav-conflict-pause": "Not Now",
    "settingsField.experimental-virtual-mouse-pointer": "Enable Virtual Mouse Pointer",
    "settingsField.experimental-virtual-mouse-pointerDescription": "Display a virtual mouse pointer that follows the cursor, for recording tutorials and demo videos.",
    "settingsGroup.experimental": "Experimental",
    "settingsGroup.experimentalDescription": "Unstable features under testing.",
    "settingsField.other-toolbox-show-all-activity-content": "Show All Activity Content in Toolbox",
    "settingsField.other-toolbox-show-all-activity-contentDescription": "When enabled, the toolbox displays all items and recipes regardless of activity settings.",
    "settingsField.shortcut-basic-production": "Basic Production",
    "settingsField.shortcut-basic-productionDescription": "Set the shortcut key for basic production; editable only when Arknights Operation Mode is off.",
    "settingsField.shortcut-copy-selection": "Copy Selection",
    "settingsField.shortcut-copy-selectionDescription": "Set the shortcut key for copying selected devices; editable only when Arknights Operation Mode is off.",
    "settingsField.shortcut-delete-device": "Delete Device",
    "settingsField.shortcut-delete-deviceDescription": "Set the shortcut key for deleting selected devices; editable only when Arknights Operation Mode is off.",
    "settingsField.shortcut-move-selection": "Move Selection",
    "settingsField.shortcut-move-selectionDescription": "Set the shortcut key for moving selected devices; editable only when Arknights Operation Mode is off.",
    "settingsField.shortcut-marquee": "Marquee Selection",
    "settingsField.shortcut-marqueeDescription": "Set the shortcut for entering or exiting marquee selection mode.",
    "settingsField.shortcut-open-toolbox": "Open Toolbox",
    "settingsField.shortcut-open-toolboxDescription": "Set the shortcut for opening the toolbox.",
    "settingsField.shortcut-paste-selection": "Paste Selection",
    "settingsField.shortcut-paste-selectionDescription": "Set the shortcut key for pasting copied devices; editable only when Arknights Operation Mode is off.",
    "settingsField.shortcut-place-conveyor": "Place Conveyor",
    "settingsField.shortcut-place-conveyorDescription": "Set the shortcut key for placing conveyors; editable only when Arknights Operation Mode is off.",
    "settingsField.shortcut-place-pipe": "Place Pipe",
    "settingsField.shortcut-place-pipeDescription": "Set the shortcut key for placing pipes; editable only when Arknights Operation Mode is off.",
    "settingsField.shortcut-quick-place": "Quick Place",
    "settingsField.shortcut-quick-placeDescription": "Set the shortcut key for opening the quick place popup.",
    "settingsField.shortcut-redo": "Redo",
    "settingsField.shortcut-redoDescription": "Set the shortcut key for redo; editable only when Arknights Operation Mode is off.",
    "settingsField.shortcut-resources-power": "Resources & Power",
    "settingsField.shortcut-resources-powerDescription": "Set the shortcut key for resources and power; editable only when Arknights Operation Mode is off.",
    "settingsField.shortcut-return-select": "Return To Select",
    "settingsField.shortcut-return-selectDescription": "Set the shortcut key for returning to select mode; editable only when Arknights Operation Mode is off.",
    "settingsField.shortcut-rotate": "Rotate",
    "settingsField.shortcut-rotate-viewport": "Rotate Viewport",
    "settingsField.shortcut-rotate-viewportDescription": "Set the shortcut key for rotating the viewport.",
    "settingsField.shortcut-pan-viewport-up": "Pan Viewport Up",
    "settingsField.shortcut-pan-viewport-upDescription": "Set the shortcut key for panning the viewport upward.",
    "settingsField.shortcut-pan-viewport-down": "Pan Viewport Down",
    "settingsField.shortcut-pan-viewport-downDescription": "Set the shortcut key for panning the viewport downward.",
    "settingsField.shortcut-pan-viewport-left": "Pan Viewport Left",
    "settingsField.shortcut-pan-viewport-leftDescription": "Set the shortcut key for panning the viewport leftward.",
    "settingsField.shortcut-pan-viewport-right": "Pan Viewport Right",
    "settingsField.shortcut-pan-viewport-rightDescription": "Set the shortcut key for panning the viewport rightward.",
    "settingsField.shortcut-rotateDescription": "Set the shortcut key for rotating preview devices; editable only when Arknights Operation Mode is off.",
    "settingsField.shortcut-save-blueprint": "Save Blueprint",
    "settingsField.shortcut-save-blueprintDescription": "Set the shortcut key for saving the current selection as a blueprint; editable only when Arknights Operation Mode is off.",
    "settingsField.shortcut-switch-device-mode": "Switch Device Mode",
    "settingsField.shortcut-switch-device-modeDescription": "Set the shortcut key for switching the current device variant; editable only when Arknights Operation Mode is off.",
    "settingsField.shortcut-synthesis": "Synthesis",
    "settingsField.shortcut-synthesisDescription": "Set the shortcut key for synthesis manufacturing; editable only when Arknights Operation Mode is off.",
    "settingsField.shortcut-toggle-base-panel": "Base Panel",
    "settingsField.shortcut-toggle-base-panelDescription": "Set the shortcut key for toggling the base panel.",
    "settingsField.shortcut-toggle-blueprint-panel": "Blueprint Panel",
    "settingsField.shortcut-toggle-blueprint-panelDescription": "Set the shortcut key for toggling the blueprint panel.",
    "settingsField.shortcut-toggle-history-panel": "History Panel",
    "settingsField.shortcut-toggle-history-panelDescription": "Set the shortcut key for toggling the history panel.",
    "settingsField.shortcut-toggle-placement-panel": "Placement Panel",
    "settingsField.shortcut-toggle-placement-panelDescription": "Set the shortcut key for toggling the placement panel.",
    "settingsField.shortcut-undo": "Undo",
    "settingsField.shortcut-undoDescription": "Set the shortcut key for undo; editable only when Arknights Operation Mode is off.",
    "settingsField.shortcut-warehouse": "Warehouse",
    "settingsField.shortcut-warehouseDescription": "Set the shortcut key for warehouse access; editable only when Arknights Operation Mode is off.",
    "settingsField.system-language": "选择语言/Choose Language",
    "settingsField.system-languageDescription": "Switch the interface language.",
    "settingsField.system-theme": "Theme",
    "settingsField.system-themeDescription": "Choose the interface theme.",
    "settingsField.sync-provider": "Sync Provider",
    "settingsField.sync-providerDescription": "Choose a sync backend for multi-device blueprint and canvas data sync.",
    "settingsGroup.arknightsOperation": "Arknights Operation",
    "settingsGroup.arknightsOperationDescription": "Options for additional behaviors under Arknights operation mode.",
    "settingsAction.experimental-clear-storage": "Clear All Storage",
    "settingsAction.experimental-clear-storage-confirm": "Clear all storage? This includes all saved blueprints, base infrastructure, toolbox configuration, module balancing library, and system settings.",
    "settingsAction.experimental-clear-storage-final-title": "Final Confirmation",
    "settingsAction.experimental-clear-storage-final-prompt": "Type \"Confirm to clear all stored data\" to continue",
    "settingsAction.experimental-clear-storage-final-placeholder": "Confirm to clear all stored data",
    "settingsAction.experimental-clear-storage-final-confirm": "Confirm Clear",
    "settingsField.experimental-storage-usage": "Storage Usage",
    "settingsField.experimental-storage-usageDescription": "Storage space used by this website in your browser.",
    "settingsGroup.debug": "Debug",
    "settingsGroup.debugDescription": "A set of debug settings.",
    "settingsGroup.display": "Display",
    "settingsGroup.displayDescription": "Rendering and frame rate related options.",
    "settingsGroup.displaySystem": "Display & System",
    "settingsGroup.displaySystemDescription": "Language, theme, and display settings.",
    "settingsGroup.game": "Game",
    "settingsGroup.gameDescription": "Display style adjustments",
    "settingsGroup.operation": "Operation",
    "settingsGroup.operationDescription": "Adjust operational logic in the simulator. Align with game habits or enable enhanced options.",
    "settingsGroup.other": "Other",
    "settingsGroup.otherDescription": "Other features and settings",
    "settingsGroup.shortcuts": "Keybindings",
    "settingsGroup.shortcutsDescription": "Edit the shortcut settings that are currently customizable.",
    "settingsGroup.system": "System",
    "settingsGroup.systemDescription": "Language, theme, and global interface preferences.",
    "settingsKeybinding.awaitingInput": "Press any key...",
    "settingsKeybinding.conflictCancel": "Cancel",
    "settingsKeybinding.conflictMessage": "The shortcut {newKey} you selected is already used by {conflictLabel}. Replace it?",
    "settingsKeybinding.conflictReplace": "Replace",
    "settingsKeybinding.conflictTitle": "Shortcut Conflict",
    "keyboardShortcutDialog.title": "Keyboard Shortcuts",
    "keyboardShortcutDialog.open": "Open Keyboard Shortcuts",
    "keyboardShortcutDialog.actionColumn": "ACTION",
    "keyboardShortcutDialog.primaryColumn": "PRIMARY",
    "keyboardShortcutDialog.secondaryColumn": "SECONDARY",
    "keyboardShortcutDialog.slotLabel": "Shortcut {index}",
    "keyboardShortcutDialog.unassigned": "Unassigned",
    "keyboardShortcutDialog.clearSlot": "Clear Shortcut",
    "keyboardShortcutDialog.resetAll": "Reset All Shortcuts",
    "keyboardShortcutDialog.resetAllConfirm": "Reset all keyboard shortcuts to their defaults?",
    "keyboardShortcutDialog.conflictPrefix": "The shortcut",
    "keyboardShortcutDialog.conflictSuffix": "is already used by {conflictLabel}. Replace it?",
    "settingsOption.ayuDark": "Ayu Dark",
    "settingsOption.ayuLight": "Ayu Light",
    "settingsOption.disabled": "Disabled",
    "settingsOption.enabled": "Enabled",
    "settingsOption.none": "None",
    "settingsOption.followSystem": "Follow System",
    "settingsOption.languageEnglish": "English (AI Translate)",
    "settingsOption.languageZhHans": "中文(简体)",
    "settingsOption.syncProvider.none": "Off",
    "settingsOption.syncProvider.webdav": "WebDAV",
    "settingsOption.syncProvider.cloudflare": "Cloudflare Worker",
    "status.edit": "Edit mode focuses on document facts and compiled topology.",
    "status.ready": "Stage1 workbench scaffold is ready.",
    "statusBar.compile": "Compile",
    "statusBar.copyright": "Endfield Industrial System Simulator",
    "statusBar.diagnostics": "Diagnostics",
    "statusBar.entities": "Entities",
    "statusBar.githubRepo": "GitHub Repository",
    "statusBar.icpFiling": "Yue ICP 2021107697-1",
    "statusBar.links": "Links",
    "statusBar.locale": "Locale",
    "statusBar.mode": "Mode",
    "statusBar.none": "None",
    "statusBar.running": "Simulation Stub",
    "statusBar.selection": "Selection",
    "statusBar.simHz": "Sim Hz",
    "statusBar.speed": "Speed",
    "statusBar.theme": "Theme",
    "statusBar.tick": "Tick",
    "statusBar.tool": "Tool",
    "statusBar.trademarkNotice": "Arknights: Endfield is a trademark of Hypergryph. This tool is not affiliated with or endorsed by Hypergryph. Image and data assets are sourced from Arknights: Endfield game data, copyright Hypergryph.",
    "statusBar.view": "View",
    "tool.belt": "Belt",
    "tool.inspect": "Inspect",
    "tool.link": "Link",
    "tool.marquee": "Marquee",
    "tool.move": "Move",
    "tool.pipe": "Pipe",
    "tool.place": "Place",
    "tool.select": "Select",
    "toolbar.canvasBottomLeft": "Canvas Bottom Left Toolbar",
    "toolbar.canvasBottomLeftSecondary": "Canvas Bottom Left Secondary Toolbar",
    "toolbar.canvasRightDock": "Canvas Right Dock Toolbar",
    "toolbar.canvasTopLeftCorner": "Canvas Top Left Corner Toolbar",
    "toolbar.tools": "Tools",
    "toolbar.views": "Views",
    "toolboxDialog.collapseBottomDock": "Collapse Toolbox",
    "toolboxDialog.dockToBottom": "Dock to Bottom",
    "toolboxDialog.empty": "There is no toolbox content to show right now.",
    "toolboxDialog.expandBottomDock": "Expand Toolbox",
    "toolboxDialog.maximize": "Maximize Toolbox",
    "toolboxDialog.resizeBottomDock": "Resize Toolbox",
    "toolboxDialog.restore": "Restore Toolbox",
    "toolboxDialog.tab.itemEncyclopedia": "Item Encyclopedia",
    "toolboxDialog.tab.moduleBalancing": "Module Balancing",
    "toolboxDialog.tab.productionPlanning": "Production Planning",
    "toolboxDialog.title": "Toolbox",
    "toolboxDialog.undock": "Undock",
    "moduleBalancing.addInput": "Add Input",
    "moduleBalancing.addInputItem": "Add Input Item",
    "moduleBalancing.addOutputItem": "Add Output Item",
    "moduleBalancing.addToStage": "Add to Stage",
    "moduleBalancing.addVersionResources": "Add Version Resources",
    "moduleBalancing.after": "after",
    "moduleBalancing.balanced": "Balanced",
    "moduleBalancing.canvas": "Canvas",
    "moduleBalancing.canvasLibrary": "Choose Canvas",
    "moduleBalancing.canvasPlaceholder": "Untitled Canvas",
    "moduleBalancing.clearStage": "Clear Stage",
    "moduleBalancing.collapseDetails": "Collapse Details",
    "moduleBalancing.collapseSection": "Collapse",
    "moduleBalancing.confirmAdd": "Add",
    "moduleBalancing.confirmEdit": "Save",
    "moduleBalancing.customModules": "Custom Modules",
    "moduleBalancing.deficit": "Deficit",
    "moduleBalancing.deleteCanvas": "Delete Canvas",
    "moduleBalancing.deleteFolder": "Delete Folder",
    "moduleBalancing.deleteFromStage": "Remove from Stage",
    "moduleBalancing.deleteModule": "Delete Module",
    "moduleBalancing.disableInfiniteInput": "Disable Infinite Supply",
    "moduleBalancing.dispatchTicketTitle": "Dispatch Ticket Demand",
    "moduleBalancing.dispatchTicketTotal": "Total",
    "moduleBalancing.dispatchTicketUnit": "dispatch tickets",
    "moduleBalancing.editModule": "Edit Module",
    "moduleBalancing.editQuantity": "Edit Quantity",
    "moduleBalancing.emptyPorts": "No items",
    "moduleBalancing.enableInfiniteInput": "Enable Infinite Supply",
    "moduleBalancing.exhaustTime": "until empty",
    "moduleBalancing.expandDetails": "Expand Details",
    "moduleBalancing.expandSection": "Expand",
    "moduleBalancing.exportCanvas": "Export Canvas",
    "moduleBalancing.filteredCanvasesEmpty": "All existing canvases contain inactive activity content and are currently hidden. Create a blank canvas or show all activity content.",
    "moduleBalancing.folder": "Folder",
    "moduleBalancing.folderName": "Folder name",
    "moduleBalancing.importCanvas": "Import Canvas",
    "moduleBalancing.importCanvasConflict": "Import Conflict",
    "moduleBalancing.importCanvasConflictMessage": "The imported canvas has a module \"{importName}\" whose settings differ from the local version. Confirming will overwrite the local module.",
    "moduleBalancing.inputItems": "Inputs",
    "moduleBalancing.loadCanvas": "Load",
    "moduleBalancing.loadOtherCanvas": "Load Another Canvas",
    "moduleBalancing.moduleIcon": "Module Icon",
    "moduleBalancing.moduleLibrary": "Module Library",
    "moduleBalancing.moduleName": "Module Name",
    "moduleBalancing.modulePlaceholder": "Untitled Module",
    "moduleBalancing.moduleNotes": "Notes",
    "moduleBalancing.moduleNotesPlaceholder": "Note module usage or constraints",
    "moduleBalancing.moveCanvas": "Move Canvas",
    "moduleBalancing.modules": "Modules",
    "moduleBalancing.nItemsMore": "{n} more items",
    "moduleBalancing.newCanvas": "New Canvas",
    "moduleBalancing.newFolder": "New Folder",
    "moduleBalancing.newModule": "New Module",
    "moduleBalancing.newStage": "New Stage",
    "moduleBalancing.noStages": "No stages yet.",
    "moduleBalancing.noSummary": "No summary data yet.",
    "moduleBalancing.none": "None",
    "moduleBalancing.outputItems": "Outputs",
    "moduleBalancing.overflowTime": "until full",
    "moduleBalancing.quantity": "Quantity",
    "moduleBalancing.recommendedCanvasEmpty": "No recommended canvases",
    "moduleBalancing.recommendedCanvasLoadFailed": "Failed to load recommended canvases",
    "moduleBalancing.recommendedCanvases": "Recommended Canvases",
    "moduleBalancing.recommendedModules": "Recommended Modules",
    "moduleBalancing.recipes": "Recipes",
    "moduleBalancing.removeInput": "Remove",
    "moduleBalancing.renameCanvas": "Rename Canvas",
    "moduleBalancing.renameFolder": "Rename Folder",
    "moduleBalancing.saveAsModule": "Save as Module",
    "moduleBalancing.saveModule": "Save Module",
    "moduleBalancing.rootFolder": "Uncategorized",
    "moduleBalancing.searchModules": "Search modules, recipes, or Pinyin",
    "moduleBalancing.stage": "Stage",
    "moduleBalancing.summary": "Summary",
    "moduleBalancing.surplus": "Surplus",
    "moduleBalancing.systemInput": "System Input",
    "moduleBalancing.systemModules": "System Recipes",
    "moduleBalancing.userCanvases": "User Canvases",
    "moduleBalancing.versionResourceEmpty": "No version resources",
    "moduleBalancing.versionResourceLoadFailed": "Failed to load version resources",
    "moduleBalancing.versionResources": "Version Resources",
    "moduleBalancing.warehouseAnalysis": "Warehouse Analysis",
    "moduleBalancing.warehouseCapacity": "Warehouse Capacity",
    "moduleBalancing.warehouseCapacityHint": "Max stock per item",
    "inspector.recipeChannelMode.automatic": "Auto",
    "inspector.recipeChannelMode.label": "Recipe channel mode",
    "inspector.recipeChannelMode.manual": "Manual",
    "productionPlanning.addRecipe": "Add Recipe",
    "productionPlanning.addSupply": "Add Supply",
    "productionPlanning.addTarget": "Add Target",
    "productionPlanning.autoRecipe": "Auto Recipe",
    "productionPlanning.autoRecipeReadonly": "Auto recipe is read-only",
    "productionPlanning.byproduct": "Byproduct",
    "productionPlanning.byproductDump": "Dump byproducts",
    "productionPlanning.byproductUse": "Use byproducts",
    "productionPlanning.calculate": "Calculate",
    "productionPlanning.chooseRecipe": "Choose Recipe",
    "productionPlanning.coverDemand": "Cover Demand",
    "productionPlanning.consumed": "Consumed",
    "productionPlanning.convertToModule": "Convert to Module",
    "productionPlanning.deviceMinimumConsumption": "Calculate device minimum consumption",
    "productionPlanning.deviceMinimumConsumptionDisabled": "Ignore",
    "productionPlanning.deviceMinimumConsumptionEnabled": "Fractional",
    "productionPlanning.deviceMinimumConsumptionRoundedUp": "Round up",
    "productionPlanning.deviceMinimumConsumptionNode": "Device minimum consumption",
    "productionPlanning.minimumConsumption": "Minimum consumption",
    "productionPlanning.devices": "Devices",
    "productionPlanning.displayMode": "Display Mode",
    "productionPlanning.emptyLines": "Add target items to start planning.",
    "productionPlanning.externalSupply": "External Supply",
    "productionPlanning.generatedModuleNotes": "Automatically generated from production planning. Targets: {targets}",
    "productionPlanning.infinite": "Infinite Supply",
    "productionPlanning.infiniteNaturalDisabled": "Natural resources use gathering recipes",
    "productionPlanning.inputSources": "Input Sources",
    "productionPlanning.modeDevice": "Device View",
    "productionPlanning.modeItem": "Item View",
    "productionPlanning.modify": "Modify",
    "productionPlanning.naturalResources": "Natural Resources",
    "productionPlanning.noRecipes": "No available recipes",
    "productionPlanning.noResult": "No result yet",
    "productionPlanning.node": "Node",
    "productionPlanning.none": "None",
    "productionPlanning.outputTargets": "Output Targets",
    "productionPlanning.perMinute": "per minute",
    "productionPlanning.produced": "Produced",
    "productionPlanning.rate": "Rate",
    "productionPlanning.recipe": "Recipe",
    "productionPlanning.remove": "Remove",
    "productionPlanning.removeExternalSupply": "Remove External Supply",
    "productionPlanning.requiredInputs": "Required Inputs",
    "productionPlanning.resetLayout": "Reset Layout",
    "productionPlanning.second_short": "s",
    "productionPlanning.selfProduce": "Self Produce",
    "productionPlanning.shared": "Shared",
    "productionPlanning.sharedDemand": "Shared demand",
    "productionPlanning.sourcePolicy": "Source Policy",
    "productionPlanning.supplies": "Supplies",
    "productionPlanning.targets": "Targets",
    "productionPlanning.totalOutputs": "Total Outputs",
    "productionPlanning.viewFlow": "Flow",
    "productionPlanning.viewMode": "View",
    "productionPlanning.viewProcess": "Process",
    "productionPlanning.viewTree": "Tree",
    "productionPlanning.waterPurifierDisabled": "Do not use",
    "productionPlanning.waterPurifierNode": "Use Purification Node",
    "productionPlanning.waterPurifierUseWhenAvailable": "Use when available",
    "timelineDialog.collapseBottomDock": "Collapse Timeline",
    "timelineDialog.dockToBottom": "Dock to Bottom",
    "timelineDialog.doNotWarnAgain": "Do not warn again this session",
    "timelineDialog.expandBottomDock": "Expand Timeline",
    "timelineDialog.maximize": "Maximize Timeline",
    "timelineDialog.preparing": "Preparing timeline, please wait...",
    "timelineDialog.resizeBottomDock": "Resize Timeline",
    "timelineDialog.restore": "Restore Timeline",
    "timelineDialog.rollbackTitle": "Confirm Timeline Move",
    "timelineDialog.rollbackWarning": "The position you moved to is before your edit. Moving there will undo that edit. Continue?",
    "timelineDialog.title": "Timeline",
    "timelineDialog.unavailable": "Timeline unavailable",
    "timelineDialog.undock": "Undock",
    "topBar.controls": "Controls",
    "topBar.device": "Device",
    "topBar.language": "Language",
    "topBar.leftPanel": "Left",
    "topBar.rightPanel": "Right",
    "topBar.screen": "Screen",
    "topBar.settings": "Settings",
    "topBar.speed": "Speed",
    "topBar.switchToOldVersion": "Switch to Old Version",
    "topBar.theme": "Theme",
    "topBar.zoom": "Zoom",
    "uiGroup.advancedManufacturing": "Advanced Manufacturing",
    "uiGroup.basicProduction": "Basic Production",
    "uiGroup.beltLogistics": "Belt Logistics Devices",
    "uiGroup.hidden": "Hidden Devices",
    "uiGroup.pipeLogistics": "Pipe Logistics Devices",
    "uiGroup.resourcePower": "Resources & Power",
    "uiGroup.warehouse": "Warehouse Access",
    "view.diagnostics": "Diagnostics",
    "view.inspector": "Right Panel",
    "view.library": "Left Panel",
    "warehouseStats.consumed": "Consumed/min",
    "warehouseStats.dialogTitle": "Warehouse Stats",
    "warehouseStats.empty": "No warehouse changes",
    "warehouseStats.item": "Item",
    "warehouseStats.more": "More",
    "warehouseStats.noResults": "No matching items",
    "warehouseStats.pin": "Pin",
    "warehouseStats.pinnedCount": "Pinned",
    "warehouseStats.produced": "Produced/min",
    "warehouseStats.runToView": "Start simulation to view",
    "warehouseStats.search": "Search items",
    "warehouseStats.stock": "Stock",
    "warehouseStats.title": "Warehouse Stats",
    "warehouseStats.unpin": "Unpin",
    "deviceStats.title": "Device Stats",
    "deviceStats.totalDevices": "Total Devices",
    "deviceStats.pipeLogisticsDevices": "Pipe Logistics Devices",
    "workbench.base.heartRepairStation": "Cardiac Remediation Station",
    "workbench.base.infraForward": "Infra-Station",
    "workbench.base.protocolCore": "Protocol Core",
    "workbench.base.rebuildHQ": "Reconstruction HQ",
    "workbench.base.refugeeOutpost": "Refugee Camp",
    "workbench.base.tianwangpingAid": "Sky King Flats Construction Site",
    "workbench.base.valley4": "Valley 4",
    "workbench.base.wuling": "Wuling",
    "workbench.base.xiranflowCloudseeder": "Xiranflow Cloudseeder Station",
    "workbench.blueprint.blueprintBadge": "Blueprint",
    "workbench.blueprint.blueprintCount.one": "blueprint",
    "workbench.blueprint.blueprintCount.other": "blueprints",
    "workbench.blueprint.cancel": "Cancel",
    "workbench.blueprint.copyClipboardFailed": "Failed to copy the blueprint to the clipboard. Check browser permissions.",
    "workbench.blueprint.copyClipboardUnavailable": "Clipboard write is unavailable in the current environment.",
    "workbench.blueprint.createFolder": "New Folder",
    "workbench.blueprint.createFolderPlaceholder": "Enter folder name",
    "workbench.blueprint.createFolderSubmit": "Create",
    "workbench.blueprint.deleteFolder": "Delete Folder",
    "workbench.blueprint.deleteFolderCancel": "Cancel Delete",
    "workbench.blueprint.deleteFolderConfirm": "Confirm Delete",
    "workbench.blueprint.detailsBase": "Base",
    "workbench.blueprint.detailsEntities": "Entities",
    "workbench.blueprint.detailsLinks": "Links",
    "workbench.blueprint.detailsTitle": "Blueprint Details",
    "workbench.blueprint.detailsUpdatedAt": "Updated",
    "workbench.blueprint.detailsVersion": "Version",
    "workbench.blueprint.editFolder": "Edit Folder",
    "workbench.blueprint.editFolderAction": "Edit Folder",
    "workbench.blueprint.emptyDirectoryDescription": "There are no blueprints or folders here yet. Save a blueprint first, or create a subfolder in this directory.",
    "workbench.blueprint.emptyDirectoryTitle": "This directory is empty",
    "workbench.blueprint.exportFileFailed": "Failed to export the blueprint file. Check browser download support.",
    "workbench.blueprint.folderBadge": "Folder",
    "workbench.blueprint.folderCount.one": "folder",
    "workbench.blueprint.folderCount.other": "folders",
    "workbench.blueprint.folderCreateFailed": "Failed to create folder. Check browser storage availability.",
    "workbench.blueprint.folderDeleteDescription": "Deleting this folder moves it into deleted items. Only empty folders can be deleted.",
    "workbench.blueprint.folderDeleteFailed": "Failed to delete the folder. Check browser storage availability.",
    "workbench.blueprint.folderDeleteNotEmpty": "This folder still contains subfolders or blueprints. Clear it before deleting.",
    "workbench.blueprint.folderNameLabel": "Folder Name",
    "workbench.blueprint.folderNameRequired": "Please enter a folder name.",
    "workbench.blueprint.folderRenameFailed": "Failed to rename the folder. Check browser storage availability.",
    "workbench.blueprint.importClipboardFailed": "Failed to read the blueprint from the clipboard. Check browser permissions.",
    "workbench.blueprint.importClipboardUnavailable": "Clipboard import is unavailable in the current environment.",
    "workbench.blueprint.importFileFailed": "Failed to read the blueprint file. Select the file again and retry.",
    "workbench.blueprint.importInvalid": "Import failed because the blueprint content is invalid or unsupported.",
    "workbench.blueprint.importSaveFailed": "Failed to import the blueprint. Check browser storage availability.",
    "workbench.blueprint.libraryLoadFailed": "Failed to load the blueprint library. Please try again.",
    "workbench.blueprint.loading": "Loading blueprint library...",
    "workbench.blueprint.noDescription": "No description",
    "workbench.blueprint.previewPending": "A visual preview is not available for this blueprint yet, but you can still review its basic details.",
    "workbench.blueprint.renameFolderSubmit": "Rename",
    "workbench.blueprint.rootFolder": "Root",
    "workbench.blueprint.systemEmptyDescription": "There are no system blueprints available yet. Preset blueprints will appear here later.",
    "workbench.blueprint.systemEmptyTitle": "The system blueprint library is empty",
    "workbench.blueprint.systemLoadFailed": "Failed to load the system blueprint library. Check the static blueprint assets.",
    "workbench.button.applyBlueprint": "Apply Blueprint",
    "workbench.button.batchSelect": "Batch Select",
    "workbench.button.beltBridge": "Belt Bridge",
    "workbench.button.beltConverger": "Belt Converger",
    "workbench.button.beltDraw": "Draw Belt",
    "workbench.button.beltSplitter": "Belt Splitter",
    "workbench.button.boxDelete": "Box Delete",
    "workbench.button.clearHistory": "Clear History",
    "workbench.button.clearSelection": "Clear Selection",
    "workbench.button.copyBlueprintToClipboard": "Copy to Clipboard",
    "workbench.button.darkInlet": "Dark Pipe Inlet",
    "workbench.button.darkOutlet": "Multi-Port Dark Outlet",
    "workbench.button.documentCommands": "Document Commands",
    "workbench.button.exportBlueprint": "Export Blueprint",
    "workbench.button.exportBlueprintToFile": "Export to File",
    "workbench.button.fillingMachine": "Filling Machine",
    "workbench.button.grinder": "Grinder",
    "workbench.button.importBlueprint": "Import Blueprint",
    "workbench.button.importBlueprintFromClipboard": "Import from Clipboard",
    "workbench.button.importBlueprintFromClipboardCompact": "Clipboard Import",
    "workbench.button.importBlueprintFromFile": "Import from File",
    "workbench.button.importBlueprintFromFileCompact": "File Import",
    "workbench.button.inspect": "Inspect",
    "workbench.button.itemInlet": "Item Inlet",
    "workbench.button.liquidTank": "Liquid Tank",
    "workbench.button.lockSelection": "Lock Selection",
    "workbench.button.pipeBridge": "Pipe Bridge",
    "workbench.button.pipeConverger": "Pipe Converger",
    "workbench.button.pipeDraw": "Draw Pipe",
    "workbench.button.pipeInlet": "Pipe Inlet",
    "workbench.button.pipeSplitter": "Pipe Splitter",
    "workbench.button.powerPost": "Power Post",
    "workbench.button.protocolStorage": "Protocol Storage",
    "workbench.button.reactorPool": "Reactor Pool",
    "workbench.button.redo": "Redo",
    "workbench.button.removeLinks": "Remove Links",
    "workbench.button.restoreLast": "Restore Last",
    "workbench.button.runtimeControls": "Runtime Controls",
    "workbench.button.sampleBus": "Warehouse Bus Sample",
    "workbench.button.sampleDarkPipe": "Dark Pipe Sample",
    "workbench.button.sampleReactor": "Reactor Sample",
    "workbench.button.saveAsBlueprint": "Save Blueprint",
    "workbench.button.saveBlueprint": "Save Blueprint",
    "workbench.button.select": "Select",
    "workbench.button.sessionActions": "Session Actions",
    "workbench.button.singleDelete": "Single Delete",
    "workbench.button.thermalPool": "Thermal Pool",
    "workbench.button.undo": "Undo",
    "workbench.button.undoDelete": "Undo Delete",
    "workbench.button.warehouseBusSegment": "Warehouse Bus Segment",
    "workbench.button.warehouseBusSource": "Warehouse Bus Source",
    "workbench.button.warehousePickupPort": "Warehouse Pickup Port",
    "workbench.button.warehouseStoragePort": "Warehouse Storage Port",
    "workbench.button.waterPump": "Water Pump",
    "workbench.history.applied": "Applied",
    "workbench.history.current": "Current",
    "workbench.history.emptyTitle": "No history yet",
    "workbench.history.initialState": "Initial State",
    "workbench.history.loading": "Loading",
    "workbench.history.redoable": "Redoable",
    "workbench.leftRail.base": "Base",
    "workbench.leftRail.blueprint": "Blueprint",
    "workbench.leftRail.delete": "Delete",
    "workbench.leftRail.history": "History",
    "workbench.leftRail.placement": "Placement",
    "workbench.leftRail.simulation": "Simulation",
    "workbench.panel.base.title": "Base",
    "workbench.panel.blueprint.title": "Blueprint Mode",
    "workbench.panel.delete.title": "Delete Mode",
    "workbench.panel.history.title": "History",
    "workbench.panel.placement.title": "Placement Mode",
    "workbench.panel.simulation.title": "Simulation",
    "workbench.power.clearOverride": "Clear Forced Load",
    "workbench.power.covered": "Forced Load",
    "workbench.power.current": "Effective Load",
    "workbench.power.infiniteSwitch": "Infinite Power",
    "workbench.power.mode": "Power Mode",
    "workbench.power.total": "Total Load",
    "workbench.powerValue.covered": "Auto (real value)",
    "workbench.powerValue.current": "20 kW",
    "workbench.powerValue.mode.infinite": "Infinite Power",
    "workbench.powerValue.mode.real": "Real Power",
    "workbench.powerValue.total": "20 kW",
    "workbench.quickPlace.emptyFavorite": "Empty favorite slot",
    "workbench.quickPlace.emptyResults": "No matching devices",
    "workbench.quickPlace.favoritesLabel": "Quick Favorites",
    "workbench.quickPlace.searchPlaceholder": "Search devices / initials",
    "workbench.quickPlace.title": "Quick Place",
    "workbench.section.advancedManufacturing": "Synthesis Manufacturing",
    "workbench.section.beltLogistics": "Belt Logistics Devices",
    "workbench.section.blueprintActions": "Blueprint Actions",
    "workbench.section.blueprintLibrary": "Blueprint Library",
    "workbench.section.deleteActions": "Delete Actions",
    "workbench.section.deleteGuard": "Recovery & Guard",
    "workbench.section.devices": "Devices",
    "workbench.section.historyActions": "History Actions",
    "workbench.section.historyLane": "History",
    "workbench.section.operation": "Actions",
    "workbench.section.pipeLogistics": "Pipe Logistics Devices",
    "workbench.section.production": "Production",
    "workbench.section.resourcePower": "Resources & Power",
    "workbench.section.warehouse": "Warehouse",
    "workbench.summary.baseTag": "Base Tag",
    "workbench.summary.buildableArea": "Buildable Area",
    "workbench.summary.expansion": "Expansion",
    "workbench.summaryValue.baseTag": "Wuling",
    "workbench.summaryValue.buildableArea": "80x80",
    "workbench.summaryValue.expansion": "T4 R4 B4 L4",
    "workbench.tab.systemBlueprints": "System Blueprints",
    "workbench.tab.userBlueprints": "User Blueprints",
    "workbench.utility.copy": "Copy",
    "workbench.utility.debugLogs": "Debug Logs",
    "workbench.utility.delete": "Delete",
    "workbench.utility.feedback": "Feedback",
    "workbench.utility.help": "Help",
    "workbench.utility.saveBlueprint": "Save Blueprint",
    "workbench.utility.settings": "Settings",
    "workbench.utility.toolbox": "Toolbox",
};

export default UI;

...
const MESSAGES: Record<AppLocale, Record<MessageKey, string>> = {
  "zh-CN": {
    ...
    "settingsField.arknightsRotateShortcut": "旋转快捷键",
    "settingsField.arknightsRotateShortcutDescription": "当前用于移动预览旋转；放置流程接入后会复用该全局键位。仅在鹰角网络操作模式关闭时可编辑。",
  ...
  "en-US": {
    ...
    "settingsField.arknightsRotateShortcut": "Rotate Shortcut",
    "settingsField.arknightsRotateShortcutDescription": "Currently used for move preview rotation; placement will reuse the same global shortcut when it is wired. Editable only when Arknights Operation Mode is off.",

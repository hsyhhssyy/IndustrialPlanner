...
const comparisons = registry.entityDefinitions.flatMap((definition) => {
  ...
  return [{ entityId: definition.id, view: viewKey, variant,
    projectPorts: view.variants[variant]!.map(({ role, index, position, yaw }) => ({ role, index, position, yaw })),
    registryPorts: definition.portGroups.filter((group) => group.isPipe).flatMap((group) => group.ports.map((port) => {
      ...
      return { groupId: group.id, portId: port.id, direction: group.direction,
        projectPlanePosition: [cell.x + .5 + delta.x * .5 - view.origin[0], cell.y + .5 + delta.y * .5 - view.origin[1]],
        projectOutward: [delta.x, delta.y] };
    })),
  }];
});
const report = { sourceArchive: 'buildings_frontend_v1.5.zip',
  policy: 'Exact Registry anchor, project-plane outward direction and input/output role. Source X maps to project x, source Z maps to project -y; no inferred reflection or index remapping.',

...
- 正式导入必须一次完成两层产物：`resources/` 保留原尺寸、原始下载字节；`public/` 从同一批原件生成全部配置比例的发布版本。只更新其中一层、漏发某个比例或混用不同来源，都不算完成。用户明确要求“仅保存原件、暂不接入”时按原件留存范围处理，不能扩大为正式导入；当前该范围的入口状态见下表。
...
- [ ] 含物流时，已生成 `logistics/baked/manifest.json`、共享颜色相位页、两张数值场、端帽、静态回退图和六项 sprite/mask；没有按颜色生成独立纹理。
...
`apply` 会保存 `backup/` 和 `apply-journal.json`；发生写入错误时自动恢复。中断或重试时，严格按 [批次恢复与归档](references/lessons-learned.md#批次恢复与归档)的状态表处理；已有 journal 的批次不能再次 `apply`，包括已恢复的批次。不要删除 journal 或绕过 Registry 差异检查。
...
- [ ] 报告发布编号、索引摘要、导入范围、原版数量、每个比例的目录和数量、未映射项及检查结果。区分“未应用”“应用失败已恢复”“应用或恢复异常”“已应用但验证未完成”“已应用但检查未通过”和“导入完成”；仅最后一种允许所有必需验收通过且无待执行项。

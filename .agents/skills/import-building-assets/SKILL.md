...
每项按“执行命令或操作 → 预期证据 → 失败动作”核对。遇到异常，先查 [导入错题本的症状索引](references/lessons-learned.md#症状索引)，只读命中的条目；条目未命中或证据不足时停止相关步骤并报告。错题本中的历史修复不能证明本次故障同因，历史报告、临时脚本归档和维护待办不能充当当前可执行入口。
...
- [ ] 在既有导入授权内一次应用 `resources` 原版、全部 `public` 版本和物流范围的 Registry 配色，并复核来源、数量及引用。应用计划允许删除受管 `public/<版本>/logistics/` 与已退役 `animations/logistics-contract2/` 中没有暂存对应物的陈旧派生产物；这些删除同样必须备份并支持恢复。网站原件移除项仍单独报告，不据此删除 Registry 物品或其他项目资产。
...
`apply` 会保存新增、替换和删除项的 `backup/` 与 `apply-journal.json`；发生写入错误时自动恢复。中断或重试时，严格按 [批次恢复与归档](references/lessons-learned.md#批次恢复与归档)的状态表处理；已有 journal 的批次不能再次 `apply`，包括已恢复的批次。不要删除 journal 或绕过 Registry 差异检查。
...
- [ ] 报告发布编号、索引摘要、导入范围、原版数量、每个比例的目录和数量、未映射项及检查结果。区分“未应用”“应用失败已恢复”“应用或恢复异常”“已应用但验证未完成”“已应用但检查未通过”和“导入完成”；只有所有必需验收通过且无待执行项时才使用“导入完成”。
- [ ] 按错题本的归档 checklist，将报告、收据、应用清单、journal 和必要的恢复副本保存到 `.temp/building-assets-import/<批次>/` 并校验，再清理暂存。检查、截图证据留在各自规定目录；任何失败或验证未完成批次都保留恢复材料。
...
- 真实故障、识别步骤和恢复分支维护在 `references/lessons-learned.md`；新增条目必须有已验证证据。先检查现有条目，修正同类问题，不重复堆叠规则；可自动化的检查应由维护任务落实为正式入口，再更新就绪状态。

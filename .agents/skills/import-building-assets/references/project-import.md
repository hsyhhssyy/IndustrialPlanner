...
2026-09-20 起权威网站输入改为根目录直接托管的 64px 分文件发布；ZIP 不再是本项目输入。2026-09-13 至 2026-09-14 的实跑使用旧来源契约，只能作为历史处理证据，不能证明当前 64px 入口就绪。正式完成状态以当前入口的实际命令、应用记录及应用后验证为准。

| 能力 | 当前实现与证据 |
| --- | --- |
| 固定发布、下载及哈希校验 | `building-assets-site-source.py` 支持根清单 v2、64px `textureProfile` 与结束锚点复查；2026-09-20 实际固定 `v1.5-20260919-212522-cst` 并校验 1871 个文件 |
| 原始 JSON 与动画来源转换 | `--metadata-only` 分离 128px 逻辑坐标与 64px 页面尺寸/`physicalRect`；同批归一化 37 个动画、15 个静态 sprite 与 53 个高度视图 |
| 静态图、动画分页、首帧及遮罩 | 发布器接收来源绝对 resolution，并以 `target / source` 计算采样；同批 `0.5 → 0.5` 已生成并通过统一验收 |
| 物流材质、数值纹理及 Registry 配色 | `publish-logistics-baked.mjs`、`sync-registry-fluid-colors.mjs` 已支持 64px 来源密度、物理帧矩形与独立配色对账 |
| 高度和裁切特效 | `publish-building-port-effects.mjs`、`building-asset-image.mjs` 按来源 `physicalRect` 读取 64px 图集，奇数逻辑宽度使用透明补边，不读取相邻帧 |
...
范围编排支持已有建筑映射的完整集合、一个或多个 `--entity` 已有映射实体、`--logistics-only` 物流集合，以及用户明确排除物流的建筑集合。完整集合直接发布烘焙物流；排除物流才执行 `defer-logistics`。局部实体与仅物流模式合并共享高度 manifest，未选建筑、特效和数值文件按原字节保留。发布收据使用 `retained=true` 区分它们与本轮新产物；局部实体收据以实体集合摘要作为文件名后缀，不能覆盖完整导入收据。不得手工拼接清单绕过范围检查。
...
本流程选择 **Extend / Compose**：网站 64px 分文件清单负责交付发现、来源锁定和物理像素，项目已有素材清单负责实体映射，现有发布器扩展来源密度参数后负责坐标变换和运行时协议。采用网站烘焙协议和参考 Shader，复用项目拓扑、仿真查询、Pixi Mesh 与数值上传；无需爬虫服务、ZIP 解包器或版本绑定的导入命令。
...
来源密度维护完成后，既有发布器继续负责像素处理，网站统一入口仍是 `import-building-assets.mjs`。其 CLI 默认路径可能指向已导入的历史来源，不能当作网站导入命令直接运行。需要组织数据时使用已验证的显式输入/输出参数；若接口仍强制依赖旧包总清单，归入首次接入维护，不由日常执行者临时扩展。维护时让处理器消费新清单解析后的视图/资源，不能伪造 ZIP、空 `ports.json` 或旧目录兼容层绕过校验。沿用相同的像素处理函数和测试，不重写已有算法。

## 64px 临时原件与绝对目标密度

网站直传源固定为 `sourceResolution=0.5`：每格 64 物理像素，但 package、spatial、动画帧和特效投影继续以每格 128 的逻辑坐标表达。`BUILDING_ASSET_PUBLISH_RESOLUTIONS` 是唯一**绝对目标密度**配置，以下只是修改配置时的示例，不能作为另一份默认值：

| 配置值 | 同一次导入必须完成的产物 |
| --- | --- |
| `[0.5]` | 64px 原件直接变换/重排为 64px 运行时产物；采样倍率 `0.5 / 0.5 = 1` |
| `[0.25]` | 64px 原件下采样为 32px 产物；采样倍率 `0.25 / 0.5 = 0.5` |
| `[0.5, 0.25]` | 同一份 64px 原件分别生成 64px 与 32px 两套独立产物 |

通过 `resolveBuildingAssetPublishTargets(outputRoot)` 取得整个列表，不得只读取第一项作为网站导入范围。原件只下载到批次 `site/` 一份，各目标从该目录生成，实际采样倍率统一为 `targetResolution / sourceResolution`；不允许目标大于来源而进行上采样。整批验收后只把派生产物应用到正式根目录 `public/3d-top-view`。第一项直接使用根目录，后续项使用 `variants/resolution-<比例>/`；每个根目录包含各自的 `sprites / sprite-masks / animations / logistics / port-effects` 等对应资产，不从一个发布目标生成另一版。

第一项是当前运行时路径使用的版本；增加其他版本不自动新增设备分档、响应式开关或运行时版本选择器。额外版本的产物和来源必须完整，但应用何时选择它们是另一个需求。

同一版颜色图和对应遮罩必须使用同一绝对目标密度。目标物理尺寸按“逻辑尺寸 × targetResolution”计算；来源物理尺寸及裁切优先使用页面尺寸和 `physicalRect`，不能再次把来源图片宽高乘 `0.5`。裁切特效的逻辑宽度可能为奇数，64px 交付因此可能出现半像素 `physicalRect`；必须按网站物理矩形协议逐帧采样、补透明边界并重新排布，不能靠 `floor/ceil` 重叠裁切，也不能直接缩放整张图集。逻辑占地、世界位置、动画帧数和逐帧时长保持原义。动画 manifest 保留原逻辑 `frameWidth / frameHeight`，用目标 `resolution` 声明实际纹理密度。
...
- `package.json`、`spatial.json` 和图集元数据共同验证画布/帧尺寸。普通图像契约为左上原点、`x=+sourceX`、`y=+sourceZ`；逻辑坐标通常为 128 像素/格，直传图片为 64 物理像素/格。遇密度或坐标契约变化先停止定位。
...
- 来源/目标密度及覆盖遮罩处理在首次接入维护中完成后，复用 `publishDeviceSprite` 的像素处理能力生成各目标版本的 WebP 和 mask。源 WebP 不重编码；发布产物执行裁切、坐标变换，并仅在目标密度低于来源时下采样。
...
每个目标版本显式传入来源 `resolution=0.5` 和绝对目标 `resolution`，不能依赖单版本函数默认值生成整个列表。动画分页、动画遮罩、静态首帧和静态遮罩都使用 `target/source` 采样率；逻辑画布、目标渲染分辨率与来源物理帧分别验证，不另写平行像素处理实现。

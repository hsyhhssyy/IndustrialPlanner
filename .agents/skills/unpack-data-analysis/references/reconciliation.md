...
- raw-table 来源读取并校验 `FactoryBuildingTable`、`FactoryBoxValveTable` 与 `FactoryFluidValveTable`；legacy 来源只使用受限映射，因此不能审计两个准入口。
...
1. 通过 `alter:` tag、已知历史别名或同名 ID，把普通 registry 实体映射到 `FactoryBuildingTable`；`log_admission` / `pipe_admission` 分别映射到 `FactoryBoxValveTable.log_conditioner` / `FactoryFluidValveTable.log_pipe_conditioner`。
2. 对解包端口统一应用已由 transmuter_1 游戏实测校准的 `localCellX = position.x`、`localCellY = depth - 1 - position.z`；禁止以当前 registry 反推这一步，避免循环论证。
3. 普通设备以方向角色、`isPipe` 和坐标构造多重集；1×1 物流阀门按端口角色与专表正交 yaw 解析 N/E/S/W 端口面后纳入同一旋转枚举。registry 变体可以是 building 全部端口的子集。
...
普通设备端口角点的 `edge` 不作为独立匹配键：用户维护的 registry 已保证端口相对布局正确，设备整体旋转时坐标和 `edge` 必须一起旋转；解包 `rotation.y` 不能为普通设备角点提供稳定的一一方向映射。只有坐标完全重合的 1×1 物流阀门按专表规则匹配 edge。
